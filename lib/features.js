...
var verifyAllowedGeo = function(featureType, geo) {
  ...
  for (var type in typeCounts) {
    if (allowed.indexOf(type) < 0) {
      errors.push('GeoJSON Type "' + type + '"" not allowed by feature "' + featureType + '".');
    }
    if (typeCounts[type] > 1) {
      errors.push('Too many features of GeoJSON type "' + type + '" found.');
    }
  }
...
var defaultAdd = function(featureType) {
  return function(db, jsonObj, cb) {
    var errors = verifyAllowedGeo(featureType, jsonObj.geo);
    if (errors) {
      return cb(errors);
    }
